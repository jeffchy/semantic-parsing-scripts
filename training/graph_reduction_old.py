	...
	for idx in nodes:
		...
		if node['label'] in single_list:
			...
			if anchor in anchor_dict:
				...
				if (anchor_equal and len(node['children'])==1 and node['children'][0]==target_node['id']):
		...
		if node['label'] in double_list and node['type']=='implicit':
			...
			if len(children)==2:
				if children[0]==children[1]:
					...
					anchor_ch_0 = nodes[children[0]]['anchors']
					anchor_ch_1 = nodes[children[1]]['anchors']
					anchor_self = node['anchors']
...
for sentence in alldicts:
	...
	# count implicit relations (useless)
	#POS_implicit_rel(sentence,wsj,rel_dict=rel_dict)
	

	pdb.set_trace()
	

	# for implicit nodes in the replace_list, check whether they have the same anchors.
	#check_implicit_anchor_conincide(sentence,reldict,replace_list=non_list)
	#count_children(sentence,rel_dict=reldict)	

	# count other implicit words is aligned in the sentence
	#count_correspond_words(sentence,wsj,reldict)

	# for implicit nodes, count whether its anchor have explicit nodes, for the implicit nodes have no corresponding anchor in the explicit nodes for all sentences, assign it as a explicit node
	#count_implicit_anchors(sentence,reldict)

	#anchor_edge_label_check(sentence,wsj,collect)
	
	#complex_word_breaking(sentence,wsj)
	# count parents and children relationships for implicit nodes
	res_sent=convert_to_conllu(sentence,wsj)
	try:
		count_parents(sentence,rel_dict=reldict)
		count_children(sentence,rel_dict=reldict2)
	except:
		pdb.set_trace()
	#write_dict.append(sentence)
	write_dict.append(res_sent)


#for key in rel_dict: print(key, rel_dict[key])
for key in reldict2:
	reldict2[key]['avg']=np.mean(reldict2[key]['children_len'])
	del reldict2[key]['children_len']
for key in reldict:
	reldict[key]['avg']=np.mean(reldict[key]['parents_len'])
	del reldict[key]['parents_len']
# loopdict=set()
# failed=set()
# for key in reldict2:
# 	for child in reldict2[key]['children']:
# 		if (key,child) not in loopdict:
# 			loopdict.add((key,child))
# 		if (child,key) in loopdict:
# 			if child==key:
# 				continue
...
# 	print('|||||',end=' ')
# 	reslist['parents'][key]=[]
# 	val_count=reldict[key]['val_count']
# 	for parent in reldict[key]['parents']:
# 		if reldict[key]['parents'][parent]/val_count>=0.95:
# 			print(parent,end=' ')
# 			reslist['parents'][key].append(parent)
# 	print()
# print('#============================================================')
...
# 	print(key,end=' ')
# 	print('|||||',end=' ')
# 	reslist['children'][key]=[]
# 	val_count=reldict2[key]['val_count']
# 	for parent in reldict2[key]['children']:
# 		if reldict2[key]['children'][parent]/val_count>=0.95:
# 			print(parent,end=' ')
# 			reslist['children'][key].append(parent)
# 	print()

#for key in remained_labels: print(key, reldict[key],'\n')
#for key in remained_labels: print(key, reldict2[key],'\n')
#for key in reldict: print(key, reldict[key],'\n')
#for key in non_list: print(key, reldict[key],'\n')
#for key in reldict2: print(key, reldict2[key],'\n')
#res=[(key,reldict2[key]['val_count']) for key in reldict2 if reldict2[key]['val_count']<50]
# countres={}
# for label in rel_dict.keys():
# 	countres['label']
#countres={label:rel_dict[label]['count'] for label in rel_dict.keys() if label!='total_count'}

# with jsonlines.open('out.jsonl', mode='w') as writer:
    # writer.write(...)
# writer= jsonlines.open('replacable_implicits.jsonl','w')
# writer.write(rel_dict)
#ast.literal_eval
#json.dump(rel_dict,open('replacable_implicits.json','w'))

# for pos in rel_dict:
# 	val_counts=rel_dict[pos]['val_counts']
# 	for label in rel_dict[pos]:
# 		if label=='val_counts':
# 			continue
# 		if rel_dict[pos][label]/val_counts>0.95:
# 			print (pos)
pdb.set_trace()
import codecs
